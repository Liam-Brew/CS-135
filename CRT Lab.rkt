...
(define (M tuplelist)
  (if (null? tuplelist)
      1
      (* (car (cdr (car tuplelist))) (M (cdr tuplelist)))
      )
  )
...
(define (CRT-exists? tuplelist)
  (if (null? tuplelist)
      #t
      (if (exist-helper (cdr tuplelist) (car (cdr (car tuplelist))))
          (CRT-exists? (cdr tuplelist))
          #f)
      )
  )

(define (exist-helper tuplelist intCompare)
  (if (null? tuplelist)
      #t
      (if (equal? 1 (EuclidAlgo intCompare (car (cdr (car tuplelist)))))
          (exist-helper (cdr tuplelist) intCompare)
          #f)
      )
  )
...
(define (CRT-helper tuplelist m)
  (if (null? tuplelist)
      0
      (+ (CRT-helper (cdr tuplelist) m)  (* (car (car tuplelist)) (/ m (car (cdr (car tuplelist)))) (mul_inv (/ m (car (cdr (car tuplelist)))) (car (cdr (car tuplelist))))))
      )
  )
